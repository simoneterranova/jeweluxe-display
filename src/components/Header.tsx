  ...
  return (
    ...
    >
      <div className="container mx-auto flex items-center justify-between">
        ...
        <nav className="hidden md:flex space-x-8 items-center">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-slate-700 hover:text-gold transition-colors duration-300 gold-underline text-sm font-medium"
            >
              {item.label}
            </a>
          ))}
        ...
        <div
          className={cn(
            'fixed inset-0 bg-cream-light/95 backdrop-blur-lg flex flex-col items-center justify-center transition-all duration-300 ease-in-out md:hidden',
          ...
          <nav className="flex flex-col items-center space-y-8 w-full px-8">
            {navItems.map((item) => (
              <a
                ...
                className="text-slate-700 hover:text-gold text-2xl font-playfair font-medium transition-colors"
              >
                {item.label}
              </a>
            ))}
